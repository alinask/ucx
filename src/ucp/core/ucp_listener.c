                                                       ...
                                                       size_t length)
{
    ...
    /* Defer server's ep creation, wireup init and user's callback to be invoked
    ...
    accept->wireup_data = ucs_malloc(length, "ucp_listener_accept_wireup_data");
    if (accept->wireup_data == NULL) {
        ucs_error("failed to allocate memory for wireup_data");
        status = UCS_ERR_NO_MEMORY;
        goto err_free_accept;
    }
